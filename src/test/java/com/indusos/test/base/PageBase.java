...
public class PageBase {
    ...
    protected WebElement scrollToElement(WebElement element) {
        WebElement ele = new WebDriverWait(TLDriver.getDriver(), 20).until(ExpectedConditions.visibilityOf(element));
        try {
            ((JavascriptExecutor) TLDriver.getDriver()).executeScript("window.scrollTo(0," + ele.getLocation().y + ")");
            waitForSync(1);
        } catch (Exception e) {
            System.out.println("scrollToElement method in PageBase - " + e);
        }
        return ele;
    }
    ...
    protected void waitForElementAttributeToBe(WebElement element, int timeInSecs, String attribute, String  value) {
        (new WebDriverWait(TLDriver.getDriver(), timeInSecs)).until(ExpectedConditions.attributeToBe(element, attribute, value));
    }
    ...
    protected WebElement getExactMatchingTextElementFromList(List<WebElement> elements, String contentText)
            throws Exception {
    ...
    protected void waitForExpectedNumberOfWindows(int num, int timeInSecs) {
        (new WebDriverWait(TLDriver.getDriver(), timeInSecs)).until(ExpectedConditions.numberOfWindowsToBe(num));
    }
